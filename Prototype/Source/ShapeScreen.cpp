...
void ShapeScreen::UpdateClearShapeBlocks()
{
	...
	for (auto it = m_clearShape.begin(); it != m_clearShape.end(); ++it) {
		...
		transform->m_scale = Vector3{ 1.0f, 1.0f, 0.1f };
		transform->SetPosition(pos);

		if (*it) {
			renderer->GetMaterial(0)->SetTexture("Assets/Default/White.png");
		}
		else {
			renderer->GetMaterial(0)->SetTexture("Assets/Default/Gray.png");
		}
...
void ShapeScreen::UpdateCurrentShapeBlocks()
{
	...
	for (auto it = m_currentShape.begin(); it != m_currentShape.end(); ++it) {
		if (*it) {
			...
			auto renderer = obj->AddComponent<MeshRenderer>();
			if (isClear) {
				renderer->GetMaterial(0)->SetTexture("Assets/Default/Green.png");
			}
			else {
				if (isInside) {
					renderer->GetMaterial(0)->SetTexture("Assets/Default/Cyan.png");
				}
				else {
					renderer->GetMaterial(0)->SetTexture("Assets/Default/Red.png");
				}
			}
			auto transform = obj->GetTransform();
			transform->SetParent(GetTransform());
			transform->m_scale = Vector3{ 0.8f, 0.8f, 0.2f };
			transform->SetPosition(pos);
