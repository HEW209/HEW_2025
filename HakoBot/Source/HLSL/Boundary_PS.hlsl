...
{
    float4 outColor = tex.Sample(samp, pin.uv); // 出力カラー
    const float4 transparentColor = float4(outColor.rgb, 0.3); // 透過色
    
    const float noiseScale = 0.3; // ノイズのスケール
    const float noiseEmi = 2; // ノイズによる発光の強さ
    const float waveScale = -50; // 波のスケール
    const float scrollSpeed = 5; // 波のスクロール速度
    const float fresnelPower = 5; // フレネル効果の強さ
    const float fresnelEmi = 1; // フレネル効果による発光の強さ
